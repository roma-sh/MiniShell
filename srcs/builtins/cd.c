...
/*   Updated: 2024/07/23 18:58:11 by eperperi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*create_previous_directory(char **env);
...
void	ft_cd(t_env **mini_env, char **args, char **env, t_env **new_export)
{
	...
	{
		temp_pwd = create_previous_directory(env);
		if (chdir(temp_pwd) != 0)
		{
			perror("chdir() error");
			exit (EXIT_FAILURE);
...
char	*create_previous_directory(char **env)
{
	...
	i = 0;
	while (env[i] != NULL && ft_strncmp("PWD=", env[i], 4) != 0)
		i++;
	rest = ft_strrchr(env[i], '/');
	len = ft_strlen(env[i]) - ft_strlen(rest);
	final = (char *)ft_malloc(len + 1);
	final = ft_substr(env[i], 4, len - 4);
	return (final);
}
...
void	change_other_envs(t_env **mini_env,t_env **new_export, char *line)
{
	...
	if (current_new_export != NULL)
	{
		...
		replace_nodes(new_export, temp_export, i, 2);
	}
}
